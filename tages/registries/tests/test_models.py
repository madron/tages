...
class RegistryModelTest(TestCase):
    ...
    def test_str_deactivated(self):
        registry = factories.RegistryFactory(
            registry_type='company', business_name='Acme', deactivation_date=date(2017, 1, 1))
        self.assertEqual(str(registry), 'Acme (Deactivated)')

    def test_description_private(self):
        registry = factories.RegistryFactory(registry_type='private', last_name='Holovaty', first_name='Adrian')
        self.assertEqual(registry.description, 'Adrian Holovaty')

    def test_description_company(self):
        registry = factories.RegistryFactory(registry_type='company', business_name='Acme')
        self.assertEqual(registry.description, 'Acme')

    def test_description_individual_company(self):
        registry = factories.RegistryFactory(registry_type='individual_company', business_name='Acme')
        self.assertEqual(registry.description, 'Acme')

    def test_description_public_administration(self):
        registry = factories.RegistryFactory(registry_type='public_administration', business_name='Acme')
        self.assertEqual(registry.description, 'Acme')
    ...
    def test_get_link(self):
        registry = factories.RegistryFactory(id=1, registry_type='company', business_name='Acme')
        self.assertEqual(
            registry.get_link(),
